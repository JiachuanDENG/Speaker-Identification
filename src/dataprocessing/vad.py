    ...
    for frame in frames:
        ...
        if not triggered:
            ...
            if num_unvoiced > 0.9 * ring_buffer.maxlen:
                ...
                triggered = False
                outputframes=b''.join([f.bytes for f in voiced_frames])
                print ('output lenght:',len(outputframes))
                yield outputframes
                ring_buffer.clear()
                voiced_frames = []
    if triggered:
        ...
        if 'id' in idDir:
            for subdir in os.listdir(os.path.join(voxdata,idDir)):
                if '.DS_Store' not in subdir:
                    for wavfile in os.listdir(os.path.join(voxdata,idDir,subdir)):
                        if 'wav' in wavfile:
                            wavfilepath=os.path.join(voxdata,idDir,subdir,wavfile)

                            audio, sample_rate = read_wave(wavfilepath)
